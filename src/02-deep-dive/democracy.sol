pragma solidity ^0.4.2;
  ...
  contract TokenRecipientContract {

      event ReceivedEther(address sender, uint amount);
      event ReceivedTokens(address _from, uint256 _value, address _token, bytes _extraData);

      function receiveApproval(address _from, uint256 _value, address _token, bytes _extraData) {
          TokenContract t = TokenContract(_token);
  ...
  contract CongressContract is OwnedContract, TokenRecipientContract {
      ...
      /* First time setup */
      function CongressContract (uint minimumQuorumForProposals, uint minutesForDebate, int marginOfVotesForMajority) payable {
      ...
      function executeProposal(uint proposalNumber, bytes transactionBytecode) {
          ...
          */
          ...
          require (now > p.votingDeadline && !p.executed && (p.proposalHash == sha3(p.recipient, p.amount, transactionBytecode)) && p.numberOfVotes >= minimumQuorum);
